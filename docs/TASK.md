...
- [ ] **T-001** GitHub 레포지토리 생성 및 브랜치 전략 정의 (`main`, `develop`, `feature/*`)
...
- [ ] **T-104** `FinanceDataReader`로 국내 주가/재무 데이터 수집 모듈 구현
  - KOSPI, KOSDAQ 주요 종목 일별 OHLCV 수집
- [ ] **T-105** 금융감독원 OpenDart API 연동 — 기업 공시 수집 모듈 구현
  - API 키 발급 및 환경변수 등록
  - 사업보고서, 주요사항 보고서 수집
- [ ] **T-106** Naver Search API 연동 — 국내 금융 뉴스 수집 모듈 구현
  - API 키 발급 및 환경변수 등록
  - 키워드 기반 최신 뉴스 수집 (날짜 필터 포함)
- [ ] **T-107** NewsAPI 연동 — 미국 금융 뉴스 수집 모듈 구현
  - API 키 발급 및 환경변수 등록

### Epic 1-3. 데이터 정제 및 구조화 모듈 개발

- [ ] **T-108** 수집 데이터 정제 공통 모듈 구현
  - HTML 태그 제거, 중복 뉴스 제거, 빈 본문 필터링
- [ ] **T-109** 메타데이터 부착 모듈 구현
  - `source`, `market`, `published_at`, `collected_at`, `category`, `sentiment` 필드 생성
  - 종목 코드 추출 로직 (`related_tickers`)
- [ ] **T-110** 정제 결과 JSON 스키마 검증 (Pydantic 모델 정의)
