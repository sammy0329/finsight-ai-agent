...
- [x] **T-226** 홈 화면 이상 감지 배지 표시
  - `AnomalyBadge` 클라이언트 컴포넌트: `/api/stocks/anomaly/[ticker]` 호출
  - 관심종목 카드 종목명 옆 `⚡ 급등 / ⚡ 급락` 배지 표시 (페이지 초기 렌더 차단 없음)
- [x] **T-227** 인사이트 페이지 이상 감지 자동 트리거
  - 페이지 진입 시 anomaly API 자동 호출 (useEffect)
  - 이상 감지 시 "⚡ 오늘 급등/급락 감지" 카드 + Z-score + 수익률 표시
  - "원인 분석하기 →" 버튼 클릭 시 관련 질문 자동 생성 및 인사이트 요청
